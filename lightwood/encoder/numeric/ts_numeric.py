...
class TsNumericEncoder(NumericEncoder):
    ...
    def __init__(self, is_target: bool = False, positive_domain: bool = False, grouped_by=None):
        ...
        self.output_size = 1  # if is_target else 3
            ...
            if self.is_target:
                if group is not None and self.normalizers is not None:
                    ...
                    if not is_none(real):
                        ...
                        vector = [real / mean] if mean != 0 else [real]
                    else:
                        pass
                        # This should raise an exception *once* we fix the TsEncoder such that this doesn't get feed `nan`  # noqa
        ...
        for vector, group in zip(encoded_values, list(zip(*dependency_data.values()))):
            if self.is_target:
                if np.isnan(vector[0]) or vector[0] == float('inf'):
                    #  or np.isnan(vector[1]) or vector[1] == float('inf'):
                    ...
                    if decode_log:
                        ...
                        if group is not None and self.normalizers is not None:
                            ...
                            mean = self._abs_mean
                            real_value = vector[0] * mean if mean != 0 else vector[0]
