...
class SkipBlock(nn.Module):
    def __init__(self, in_size: int, skip: bool):
...
class ResidualNet(torch.nn.Module):
    def __init__(self,
        ...
        blocks = [SkipBlock(input_size, False)]
        blocks += [SkipBlock(input_size, True)] * 6
...
class Residual(BaseMixer):
    ...
    def _max_fit(self, train_dl, dev_dl, criterion, optimizer, scaler, stop_after, return_model_after):
        ...
        for epoch in range(1, return_model_after + 1):
            ...
            # If the error is getting worst
            stop_due_to_error = False
            if len(running_errors) >= 10:
                if epoch / 2 > epochs_to_best:
                    if np.mean(running_errors[-5:]) > best_dev_error:
                        stop_due_to_error = True

            '''
            delta_stop_epochs = 30
            vote_to_break = 0
            if len(running_errors) >= delta_stop_epochs:
                delta_mean = np.average([running_errors[-i - 1] - running_errors[-i]
                                         for i in range(1, delta_stop_epochs)],
                                        weights=[(1 / 2)**i for i in range(1, delta_stop_epochs)])
                if delta_mean <= 0:
                    vote_to_break += 1
                    running_errors = [running_errors[-1]]

            if vote_to_break > 0:
                if running_errors[-1] > best_dev_error * 2:
                    vote_to_break += 1
